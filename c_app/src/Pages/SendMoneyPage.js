...
const SendMoneyPage = () => {
  ...
  const handleAmountChange = (e) => {
    ...
    if (isNaN(inputAmount)) {
      ...
      setErrorMessage(
        `エラー: 送金金額が送金上限額(${maxAmount}円)を超えています。`
      );
      setIsButtonEnabled(false);
    } else {
      setErrorMessage("");
  ...
  const handleTransfer = async () => {
    ...
    const updatedRecipientBalance =
      parseFloat(recipient.yokin) + transferAmount;
    ...
    try {
      await fetch(`http://localhost:3010/users/${senderId}`, {
        ...
        body: JSON.stringify({
          ...sender,
          yokin: updatedSenderBalance.toString(),
        }),
      });
      ...
      await fetch(`http://localhost:3010/users/${recipient.id}`, {
        ...
        body: JSON.stringify({
          ...recipient,
          yokin: updatedRecipientBalance.toString(),
        }),
      });
      ...
      // alert(
      //   `送金完了！${transferAmount}円を ${recipient.username} さんに送金しました。`
      // );
      setAmount("");
      setMessage("");
      setIsButtonEnabled(false);
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            width: "100%",
            padding: "10px",
            backgroundColor: isButtonEnabled ? "#dc3545" : "#cccccc",
            color: "#fff",
            border: "none",
            borderRadius: "5px",
